...
#### SNI override (connect-IP vs cert hostname)

To connect to an IP literal while keeping TLS SNI and certificate
validation on the broker's FQDN, set `serverName`:

```swift
mqtt.host = "203.0.113.10"               // connect address (IP literal)
mqtt.serverName = "broker.example.com"   // TLS SNI / cert hostname
mqtt.enableSSL = true
```

The same property is available on `CocoaMQTT5`. For
`CocoaMQTTWebSocket` the property exists for API symmetry but the
default `URLSessionWebSocketTask`-based connection cannot honor it --
see the property documentation for the workaround via a custom
`ConnectionBuilder`.
